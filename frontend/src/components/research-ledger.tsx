...
export function ResearchLedger({ research, status = "unavailable", onEnrich }: { research?: ResearchBundle; status?: ResearchSnapshotStatus; onEnrich?: () => void }) {
  ...
  const legacyNoEvidence = status === "fallback"
    && research.status === "fallback"
    && research.sources.length === 0
    && research.warnings.some((warning) => warning.includes("没有可核验来源"));
  const effectiveStatus: ResearchSnapshotStatus = legacyNoEvidence ? "no_evidence" : status;
  ...
  return <section className="research-ledger">
    ...
    {effectiveStatus === "stale" && <p className="research-ledger__warning" role="status"><AlertTriangle size={15} />正文已经修改，当前证据绑定继承自上一版本，需重新核验后才能视为最新结论。</p>}
    {effectiveStatus === "no_evidence" && <p className="research-ledger__notice" role="status"><FileQuestion size={15} />本次报告未使用联网检索，也没有可核验的材料；正文按正常流程生成，但关键判断暂时没有外部证据绑定。</p>}
    {effectiveStatus === "fallback" && <p className="research-ledger__warning" role="status"><AlertTriangle size={15} />报告生成路径发生降级；用户输入与已有来源已保留，但关键判断尚未完成逐条提取。</p>}
    {effectiveStatus === "extraction_failed" && <p className="research-ledger__warning" role="status"><AlertTriangle size={15} />已有来源，但证据账本提取失败；正文仍可阅读，建议重新补充材料或再次核验。</p>}
    {onEnrich && (effectiveStatus === "fallback" || effectiveStatus === "extraction_failed" || effectiveStatus === "no_evidence" || effectiveStatus === "stale") && <button className="secondary-button research-enrich-trigger" type="button" onClick={onEnrich}><FilePlus2 size={16} />补充信息与证据</button>}
    <section className="research-reliability" aria-label="研究可靠性指标"><div><span className="eyebrow">可解释质量评估</span><h3>资料可靠性</h3><p>各项指标分别说明证据状况，不用单一总分掩盖缺口。</p></div><dl>{reliabilityMetrics.map((metric) => <div key={metric.label}><dt>{metric.label}</dt><dd>{metric.value}</dd><small>{metric.detail}</small></div>)}</dl></section>

...
function ProjectDirectory({ state }: { state: AppState }) {
...
export function ProjectsScreen() {
  const { state } = useAppStore();
  return <><PageHeading eyebrow="项目组合" title="项目" action={<Link className="primary-button" href="/analysis">新建项目 <ArrowRight size={16} /></Link>}>以项目为容器，让材料、分析任务、报告和判断始终保留在同一个上下文中。</PageHeading><ProjectDirectory state={state} /></>;
}
...
export function SettingsScreen() {
  ...
  return <><PageHeading eyebrow="应用偏好" title="设置">配置界面主题和默认导出方式。偏好保存在当前工作空间。</PageHeading><section className="settings-stack"><div><span className="eyebrow">分析引擎</span><h2>固定报告工作流</h2><p>自由输入会自动进入证据整理、结构分析、分节写作和报告导出流程。</p></div><div><span className="eyebrow">显示主题</span><h2>{state.settings.theme === "light" ? "明亮研究工作台" : "深色研究工作台"}</h2><p>主题立即作用于整个分析空间，并在下次打开时恢复。</p><label className="setting-control"><span>显示主题</span><select aria-label="显示主题" value={state.settings.theme} onChange={(event) => updateSettings({ theme: event.target.value as "light" | "dark" })}><option value="light">明亮</option><option value="dark">深色</option></select></label></div><div><span className="eyebrow">默认导出</span><h2>{state.settings.defaultExport === "markdown" ? "Markdown 报告" : "HTML 报告"}</h2><p>报告阅读器的导出按钮将直接使用这一格式。</p><label className="setting-control"><span>默认导出格式</span><select aria-label="默认导出格式" value={state.settings.defaultExport} onChange={(event) => updateSettings({ defaultExport: event.target.value as "markdown" | "html" })}><option value="markdown">Markdown</option><option value="html">HTML</option></select></label></div></section></>;
}

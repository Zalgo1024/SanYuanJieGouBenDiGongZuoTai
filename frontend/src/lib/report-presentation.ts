...
export type ReportReadingMode = "quick" | "standard" | "research";
...
const quickHeadingPattern = /(事实摘要|事件事实|关键发现|核心发现|核心判断|核心结论|结论|行动建议|方案|决策)/;

export function selectSectionsForReadingMode(sections: ReportSection[], mode: ReportReadingMode) {
  if (mode === "research") return sections;
  if (mode === "standard") return sections.slice(0, 6);
  const preferred = sections.filter((section) => quickHeadingPattern.test(section.heading));
  return (preferred.length ? preferred : sections).slice(0, 3);
}

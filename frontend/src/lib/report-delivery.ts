...
export type ReportArtifactKind = "word" | "pdf" | "pptx" | "zip";
...
function normalizeResearchStatus(value: unknown): ResearchSnapshotStatus {
  return value === "verified" || value === "fallback" || value === "no_evidence" || value === "extraction_failed" || value === "stale" ? value : "unavailable";
}
...
export function normalizeResearchBundle(value: unknown): ResearchBundle | undefined {
  ...
  const warnings = strings(root.warnings);
  const legacyNoEvidence = root.status === "fallback"
    && warnings.some((warning) => warning.includes("没有可核验来源"));
  return {
    schemaVersion: typeof root.schema_version === "string" ? root.schema_version : "1.0",
    // Older reports stored the no-source case as fallback. Interpret that
    // legacy payload deterministically without rewriting historical data.
    status: legacyNoEvidence
      ? "no_evidence"
      : root.status === "verified" || root.status === "no_evidence" || root.status === "extraction_failed"
        ? root.status
        : "fallback",
...
export async function downloadReportArtifact(
  ...
  const extension = kind === "word" ? "docx" : kind === "pptx" ? "pptx" : kind === "zip" ? "zip" : "pdf";

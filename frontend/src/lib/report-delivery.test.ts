...
describe("report delivery adapter", () => {
  ...
  it("recognizes legacy no-evidence ledgers without calling them generation fallback", () => {
    const research = normalizeResearchBundle({
      schema_version: "1.2",
      status: "fallback",
      sources: [],
      claims: [{ id: "c1", text: "用户问题", claim_type: "user_input", significance: "key", evidence_ids: [] }],
      relations: [],
      gaps: [],
      metrics: {},
      warnings: ["当前没有可核验来源，已跳过结构化证据提取"],
    });

    expect(research?.status).toBe("no_evidence");
  });
});

...
def build_materials(
    ...
    def _push(url: str, title: str, snippet: str, f: dict) -> None:
        # 只有抓取成功且确实拿到正文的 URL 才能进入 sources。此前调用方
        # 用 ``fetched_map.get(url) or {}`` 处理抓取失败时，空字典没有
        # ``error`` 字段，导致失效链接被误标为 kept=True 并进入报告附录。
        usable_text = bool(str(f.get("text") or "").strip())
        kept = url not in excluded and not f.get("error") and usable_text
        items.append(
            {

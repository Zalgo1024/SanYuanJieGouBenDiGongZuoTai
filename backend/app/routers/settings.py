...
class LlmTestIn(BaseModel):
    ...
    api_key: str | None = None  # 可空：配合 profile_id 使用已保存密钥测试
    base_url: str | None = None
    model: str | None = None
...
def post_llm_test(req: LlmTestIn):
    """最小代价验证一套 LLM 连接（发 1 个 8-token 请求）；不保存任何配置。"""
    from app.llm_client import test_connection
    from app.llm_settings_store import load_profile_settings

    api_key = (req.api_key or "").strip()
    if not api_key and req.profile_id:
        try:
            api_key = (load_profile_settings(req.profile_id).get("api_key") or "").strip()
        except ValueError:
            pass  # profile_id 非法时按未提供处理
    return test_connection(api_key, (req.base_url or "").strip(), (req.model or "").strip())

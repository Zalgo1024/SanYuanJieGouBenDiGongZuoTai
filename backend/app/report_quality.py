...
# 核心冲突点是一个语义列表。LLM 可能遵循提示词使用数字编号，也可能使用
# 合法的 Markdown 短横线/星号列表；这里只在该专门章节内计数，避免放宽全篇
# 列表规则后把普通条目误算为冲突点。
_CONFLICT_ITEM_RE = re.compile(
    r"^\s*(?:[-*]\s+|(?:\*\*)?[（(]?\s*(?:[一二三四五六七八九十百]+|\d+)\s*[）)、.．:：]\s*(?:\*\*)?|[①②③④⑤⑥⑦⑧⑨⑩]\s*)",
    re.MULTILINE,
)
_GENERIC_ACTORS = {
...
def _repeated_paragraphs(markdown: str) -> list[str]:
    prose = _DIAGRAM_RE.sub("", markdown or "")
    seen: set[str] = set()
    repeated: list[str] = []
...
def evaluate_report_quality(
    ...
    *,
    ...
    strict_unverified_facts: bool = False,
    input_text: str = "",
    ...
    conflict_count = len(_CONFLICT_ITEM_RE.findall(conflicts))
    ...
    if used_web_sources and not _MARKDOWN_LINK_RE.search(appendix):
        add("source_links", "warning", "使用联网来源时，建议附录包含 Markdown 来源链接。", "附录")
    ...
    fact_items = [line.strip() for line in facts.splitlines() if _LIST_ITEM_RE.match(line.strip())]
    if used_web_sources:
        # 来源标记/来源名校验：LLM 偶尔漏标或表述不一致，不阻塞报告生成，
        # 降为 warning（error 只留给结构性硬伤）。
        unattributed = [line for line in fact_items if "来源：" not in line and "来源:" not in line]
        if unattributed:
            add(
                "fact_source_attribution",
                "warning",
                f"联网报告有 {len(unattributed)} 条事实摘要缺少来源标记。",
                "事实摘要",
            )
        if source_names:
            known_names = [name.strip().lower() for name in source_names if name.strip()]
            unknown_sources = []
            for line in fact_items:
                match = re.search(r"来源\s*[:：]\s*([^）)]+)", line)
                if not match:
                    continue
                label = match.group(1).strip().lower()
                if not any(label in name or name in label for name in known_names):
                    unknown_sources.append(match.group(1).strip())
            if unknown_sources:
                add(
                    "unknown_fact_source",
                    "warning",
                    "事实摘要引用了本次材料中不存在的来源："
                    + "、".join(dict.fromkeys(unknown_sources))
                    + "。",
                    "事实摘要",
                )
    elif strict_unverified_facts:
        unattributed = [line for line in fact_items if "来源：" not in line and "来源:" not in line]
        if unattributed:
            add(
                "fact_source_attribution",
                "error",
                f"无可核验来源时，事实摘要有 {len(unattributed)} 条未明确标注为用户输入。",
                "事实摘要",
            )
        marker_re = re.compile(
            r"(?:20\d{2}(?:年|[-/.]\d{1,2}(?:月|[-/.]\d{1,2}(?:日)?)?)?|"
            r"\d+(?:\.\d+)?\s*(?:万|亿|万元|亿元|元|%|％|分|万人|万次|亿美元))"
        )
        invented = [
            marker
            for line in fact_items
            for marker in marker_re.findall(line)
            if marker not in (input_text or "")
        ]
        if invented:
            add(
                "unverified_fact_detail",
                "error",
                "无可核验来源时，事实摘要包含用户输入未提供的具体日期、数量或指标："
                + "、".join(dict.fromkeys(invented))
                + "。",
                "事实摘要",
            )
        user_source_labels = {"用户输入", "用户提供", "题述", "原始输入"}
        external_attributions = [
            match.group(1).strip()
            for line in fact_items
            for match in [re.search(r"来源\s*[:：]\s*([^）)]+)", line)]
            if match
            and not any(
                allowed in match.group(1).strip().lower()
                for allowed in user_source_labels
            )
        ]
        if external_attributions:
            add(
                "unverified_fact_source",
                "error",
                "无可核验来源时，事实摘要不能归因于外部公告、媒体或平台："
                + "、".join(dict.fromkeys(external_attributions))
                + "。",
                "事实摘要",
            )
    ...
    repeated = _repeated_paragraphs(markdown)
    if repeated:
        add(
            "repeated_analysis",
            "warning",
            f"发现 {len(repeated)} 处重复分析段落；同一判断应只完整论证一次，其余位置用一句话引用。",
        )

    ...
    *,
    ...
    if found_placeholders:
        add(
            "failure_placeholder",
            "error",
            "报告残留失败占位句：" + "、".join(found_placeholders),
        )
    ...
    if not 3 <= conflict_count <= 5:
        add(
            "conflict_count",
            "error",
            f"核心冲突点应为 3 至 5 条，当前为 {conflict_count} 条。",
            "核心冲突点",
        )
    ...
    if not flow or not judgment or not terminal or not 3 <= terminal_count <= 5:
        add(
            "conclusion_structure",
            "error",
            "结论必须包含汇流段、核心判断和 3 至 5 条博弈终局预判。",
            "结论",
        )

    actions = _find_section(sections, "行动建议")
    if not re.search(r"^###\s+(?:对|面向)\S+", actions, re.MULTILINE):
        add("action_grouping", "error", "行动建议未按主体分块。", "行动建议")

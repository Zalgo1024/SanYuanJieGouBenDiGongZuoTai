...
def test_api_rejects_freeform_without_llm_before_creating_task(client, monkeypatch):
    ...
    monkeypatch.setattr("app.routers.analyze.llm_is_available", lambda _cfg=None: False)
...
def test_api_persists_selected_llm_for_freeform_auto(client, monkeypatch):
    ...
    monkeypatch.setattr("app.routers.analyze.llm_is_available", lambda _cfg=None: True)
    response = client.post(
        "/api/analyze",
        json={
            "title": "自动路由测试",

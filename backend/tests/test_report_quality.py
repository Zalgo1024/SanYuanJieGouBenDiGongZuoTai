...
def test_web_report_requires_source_marker_on_each_fact_item():
    """联网报告事实缺来源标记：降级为 warning，不阻塞生成。"""
    markdown = _valid_report().replace("（来源：行业说明）", "")
    result = evaluate_report_quality(markdown, analysis_type="case", used_web_sources=True)

    assert result.valid is True
    assert "fact_source_attribution" in _codes(result, "warning")


def test_web_report_rejects_source_name_missing_from_material_catalog():
    """联网报告来源名不在材料清单：降级为 warning，不阻塞生成。"""
    result = evaluate_report_quality(
        _valid_report(),
        analysis_type="case",
        used_web_sources=True,
        source_names=["其他公告", "其他说明"],
    )

    assert result.valid is True
    assert "unknown_fact_source" in _codes(result, "warning")


def test_web_report_missing_markdown_source_links_is_warning():
    """联网报告附录缺 Markdown 链接：降级为 warning，不阻塞生成（LLM 可后补）。"""
    markdown = _valid_report().replace(
        "[正式公告](https://example.com/notice)", "正式公告"
    ).replace("[行业说明](https://example.com/industry)", "行业说明")
    result = evaluate_report_quality(markdown, analysis_type="case", used_web_sources=True)

    assert result.valid is True
    assert "source_links" in _codes(result, "warning")


def test_no_source_report_rejects_unprovided_numeric_fact_details():
    markdown = _valid_report().replace(
        "1. 监管部门发布了新规则。",
        "1. 2027年8月12日监管部门发布了新规则，金额为3800万元。",
    )
    result = evaluate_report_quality(
        markdown,
        analysis_type="case",
        used_web_sources=False,
        strict_unverified_facts=True,
        input_text="分析监管部门与经营主体之间的规则争议。",
    )

    assert result.valid is False
    assert "unverified_fact_detail" in _codes(result, "error")


def test_no_source_report_requires_user_input_fact_attribution():
    markdown = _valid_report().replace("（来源：正式公告）", "", 1)
    result = evaluate_report_quality(
        markdown,
        analysis_type="case",
        used_web_sources=False,
    ...
    assert result.valid is False
    assert "fact_source_attribution" in _codes(result, "error")


def test_conflicts_written_as_markdown_bullets_are_counted():
    markdown = _valid_report().replace(
        "1. 【监管部门】对【经营主体】在【执行标准】上的张力：双方对边界理解不同。\n"
        "2. 【经营主体】对【监管部门】在【合规成本】上的张力：成本承担与收益不对称。\n"
        "3. 【监管部门】对【经营主体】在【反馈速度】上的张力：制度节奏慢于经营变化。",
        "- **执行标准冲突**：监管部门与经营主体对边界理解不同。\n"
        "- **合规成本冲突**：经营主体承担的成本与收益不对称。\n"
        "- **反馈速度冲突**：制度节奏慢于经营变化。",
    )

    result = evaluate_report_quality(
        markdown, analysis_type="case", used_web_sources=True
    )

    assert "conflict_count" not in _codes(result, "error")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # source_links 已降级为 warning（附录缺 Markdown 链接不阻塞生成），见 warning 组
